...
from jshop_runner_lib import JSHOP2Error, resolve_course_layout, run_jshop2, write_result_json


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a professor-style JSHOP2 domain directory with fixed domain/problem names."
    )
    parser.add_argument(
        "--domain-dir",
        required=True,
        help="Directory containing an extensionless domain file named like the directory plus a `problem` file.",
    )
...
def main() -> int:
    args = parse_args()
    try:
        domain_path, problem_path = resolve_course_layout(args.domain_dir)
        result = run_jshop2(
            domain_path,
            problem_path,
    ...
    print(f"Course layout: {Path(args.domain_dir).resolve()}")
